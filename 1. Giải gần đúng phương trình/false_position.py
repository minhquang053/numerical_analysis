...
def secant(init_p_0: float, init_p_1: float, TOL: float, N_0: int) -> float:
    ...
    '''
    assert f(init_p_0) * f(init_p_1) < 0, f"f({init_p_0}) * f({init_p_1}) must be negative"
    print("\nStart iterating...")
    p_0 = init_p_0
    p_1 = init_p_1
    q_0 = f(p_0)
...
if __name__ == '__main__':
    print("False Position: To ﬁnd a solution to f(x) = 0 given the continuous function f on the interval [p_0,p_1] where f(p_0) and f(p_1) have opposite signs")
    p_0 = float(input("p_0 = "))
    p_1 = float(input("p_1 = "))
